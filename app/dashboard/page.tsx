...
export default function DashboardPage() {
  ...
  const handleDownload = async () => {
    ...
    const alreadyDownloaded = selectedLeads.filter(leadId =>
      downloadHistory.some((h: any) => h.leads?.id === leadId)
    )
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            { label: 'Available Leads', value: filteredLeads.length, icon: Database, color: 'blue' },
            { label: 'Selected', value: selectedLeads.length, icon: CheckCircle, color: 'green' },
            { label: 'Downloaded', value: downloadHistory.length, icon: History, color: 'purple' },
            { label: 'Cost', value: selectedLeads.filter(id => !downloadHistory.some((h: any) => h.leads?.id === id)).length, icon: Coins, color: 'orange' }
          ].map((stat, i) => (
            <div key={i} className="bg-white rounded-3xl shadow-sm p-6 border">
        ...
        <div className="bg-white rounded-3xl shadow-sm p-4 border flex justify-between items-center">
          ...
          <button onClick={handleDownload} disabled={selectedLeads.length === 0} className={`px-6 py-2.5 rounded-2xl font-semibold flex items-center gap-2 ${selectedLeads.length > 0 ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-lg' : 'bg-gray-300 text-gray-500'}`}>
            ...
            Download ({selectedLeads.filter(id => !downloadHistory.some((h: any) => h.leads?.id === id)).length} credits)
          </button>
        </div>
              ...
              <tbody className="divide-y">
                {filteredLeads.length === 0 ? (
                  ...
                  filteredLeads.map(lead => {
                    const downloaded = downloadHistory.some((h: any) => h.leads?.id === lead.id)
                    return (
        ...
        {downloadHistory.length > 0 && (
          <div className="bg-white rounded-3xl shadow-sm p-6 border">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {downloadHistory.map((record: any, idx) => (
                <div key={idx} className="flex justify-between p-4 bg-gray-50 rounded-2xl">
                  <div>
                    <p className="font-semibold">{record.leads?.company_name}</p>
                    <p className="text-sm text-gray-600">{record.leads?.contact_name} - {record.leads?.industry}</p>
                  </div>
                  <div className="text-right">
